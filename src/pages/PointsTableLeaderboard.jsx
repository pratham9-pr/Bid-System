...
export default function PointsTableLeaderboard({ transparentBg: propTransparentBg } = {}) {
  ...
  const activeTeamsList = (teams && teams.length > 0 ? teams : TEAMS_CONFIG).map((t, idx) => {
    ...
    const { totalCount = 0, remainingSlots = MAX_ROSTER_SIZE, isFull = false } = getTeamFullRoster(teamId, players);
            ...
            {sortedTeams.map((team, index) => {
              ...
              return (
                <motion.div
                  ...
                  className={`w-full py-3.5 px-6 rounded-2xl flex items-center justify-between transition-all duration-300 relative overflow-hidden
                    ${
                      isRank1
                ...
                >
                  ...
                  <div className="w-32 sm:w-36 text-right pr-4 flex-shrink-0">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <span
                        className={`font-black italic text-lg sm:text-2xl tracking-tight tabular-nums
