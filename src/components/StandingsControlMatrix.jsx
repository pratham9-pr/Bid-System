...
export function StandingsControlMatrix({ teams = [], onRefresh }) {
  ...
  return (
    <div className="space-y-6 font-rajdhani">
      ...
      {/* ── Dynamic Team Matrix Grid ─────────────────────────────────── */}
      {!teams || teams.length === 0 ? (
        <div className="card p-12 text-center text-muted font-inter text-sm rounded-2xl bg-[#11131a] border border-white/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          {teams.map((team) => (
            <TeamCard
              key={team.id}
              team={team}
              item={standingsData[team.id] || {
                id: team.id,
                name: getTeamDisplayName(team.id, team.team_name || team.name),
                owner: getTeamOwner(team.id, team.owner_name || team.owner),
                logo: team.logo_url || team.logo || getTeamLogo(team.id),
                matches_played: 0,
                wins: 0,
                losses: 0,
                score_diff: 0,
              ...
              onStep={handleStep}
              onAutoCalcPoints={handleAutoCalcPoints}
...
// ─── Dynamic Team Matrix Card Subcomponent ────────────────────────────────────
export function TeamCard({
  ...
  const theme = getTeamTheme(liveTeamId);
  const accentBorder = theme?.badge ? `border-${theme.badge.replace('bg-', '')}/40 hover:border-${theme.badge.replace('bg-', '')}` : 'border-white/15 hover:border-amber-400';
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-black/50 p-2.5 rounded-lg border border-emerald-500/20 flex flex-col items-center justify-between">
          ...
          <div className="flex items-center gap-1.5 my-1.5">
            <button
              onClick={() => onStep?.(liveTeamId, 'wins', -1)}
            ...
            <input
              type="number"
              value={item.wins}
              onChange={(e) => onFieldChange?.(liveTeamId, 'wins', e.target.value)}
        ...
        <div className="bg-black/50 p-2.5 rounded-lg border border-rose-500/20 flex flex-col items-center justify-between">
          ...
          <div className="flex items-center gap-1.5 my-1.5">
            <button
              onClick={() => onStep?.(liveTeamId, 'losses', -1)}
              className="w-6 h-6 rounded bg-surface-800 hover:bg-surface-700 text-white font-black text-xs flex items-center justify-center border border-white/10"
            >
              -
            </button>
            <input
              type="number"
              value={item.losses}
              onChange={(e) => onFieldChange?.(liveTeamId, 'losses', e.target.value)}
        ...
        <div className="bg-black/50 p-2.5 rounded-lg border border-white/10 flex flex-col items-center justify-between">
          ...
          <div className="flex items-center gap-1.5 my-1.5">
            <button
              onClick={() => onStep?.(liveTeamId, 'score_diff', -1)}
              className="w-6 h-6 rounded bg-surface-800 hover:bg-surface-700 text-white font-black text-xs flex items-center justify-center border border-white/10"
            >
              -
            </button>
            <input
              ...
              onChange={(e) => onFieldChange?.(liveTeamId, 'score_diff', e.target.value)}
        ...
        <div className="bg-black/70 p-2.5 rounded-lg border-2 border-amber-500/50 flex flex-col items-center justify-between shadow-[0_0_15px_rgba(245,158,11,0.15)]">
          ...
          <div className="flex items-center gap-1.5 my-1.5">
            <button
              onClick={() => onStep?.(liveTeamId, 'points', -1)}
            ...
            <input
              type="number"
              value={item.points}
              onChange={(e) => onFieldChange?.(liveTeamId, 'points', e.target.value)}
      ...
      <div className="flex items-center justify-between text-[11px] font-bold text-slate-400 pt-1">
        ...
        <button
          onClick={() => onAutoCalcPoints?.(liveTeamId)}
...
export const StandingsMatrix = StandingsControlMatrix;
export default StandingsControlMatrix;
